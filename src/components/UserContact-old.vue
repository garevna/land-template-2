...
export default {
    ...
    return {
      ...
      items: {
        ...
        state: {
          ...
          color: '',
          validationIcon: '',
          validator () { this.error = this.available.indexOf(this.value) === -1 }
        }
      }
    }
  },
  ...
  methods: {
    ...
    async sendUserRequest () {
      ...
      const response = await fetch('https://dka.dgtek.net/api/frontend/mail', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          phone: this.items.phone ? this.items.phone.value : '...',
